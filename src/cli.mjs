...
function preguntarPorProyecto(config, ctx, cwd, payload) {
  ...
  const lineas = [
    ...
    `Carpeta: ${ctx.cwd}`,
    '',
    'PREGUNTALE AL USUARIO AHORA con `AskUserQuestion`, la herramienta de preguntas de Claude',
    'Code, y EJECUTÁ SU RESPUESTA vos mismo. No imprimas un menú de texto pidiendo que conteste',
    'por escrito, y no se lo menciones al pasar: hacé la pregunta y actuá.',
    '',
    '  header: "ClickUp"',
    '  ¿Querés que el trabajo de este proyecto quede registrado como tareas en ClickUp?',
    '',
    '  Sí, generá tareas acá   → corré `/clickup-setup` vos mismo, ahora.',
    `  No, este no usa ClickUp → corré \`${ctx.cli} project exclude --reason "<lo que dijo>"\``,
    '  Ahora no                → no corras nada. Seguí con el trabajo.',
    '',
